      ...
      <main class="max-w-7xl mx-auto w-full p-4 md:p-6 grid grid-cols-1 lg:grid-cols-4 gap-6 flex-1 overflow-hidden">
        ...
        <div class="lg:col-span-1 space-y-4 overflow-y-auto">
          <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm transition-all hover:shadow-md">
            <h2 class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4 text-right">ניהול מסמכים</h2>
            ...
            <div 
              ...
              <p class="text-sm font-semibold text-slate-700">העלאת דוח PDF</p>
              <p class="text-[10px] text-slate-400 mt-1">גרור לכאן או לחץ לבחירה</p>
            </div>
            ...
            @if (selectedFile()) {
              <div class="mt-4 p-3 bg-blue-50 border border-blue-100 rounded-xl animate-in fade-in slide-in-from-top-2 duration-300">
                <div class="flex items-center gap-2 mb-3">
                  ...
                  <span class="text-xs font-bold text-blue-900 truncate flex-1 text-right">{{ selectedFile()?.name }}</span>
                </div>
                <button 
                  (click)="upload()" 
                  ...
                  {{ isUploading() ? 'מעבד מסמך...' : 'אנדקס והתחל ניתוח' }}
                </button>
              </div>
            }
          </div>
          ...
          <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
            <h2 class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4 text-right">תובנות מהירות</h2>
            <div class="space-y-2">
              @for (suggestion of suggestions; track suggestion) {
                <button 
                  (click)="setQueryFromSuggestion(suggestion)"
                  [disabled]="!isIndexed()"
                  class="w-full text-right text-[11px] p-2.5 rounded-xl border border-slate-100 text-slate-600 hover:bg-slate-50 hover:border-blue-200 transition-all disabled:opacity-30 disabled:hover:bg-transparent">
                  {{ suggestion }}
                </button>
              }
            </div>
          ...
          <div #chatBox class="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 bg-slate-50/30">
            @if (messages().length === 0) {
              <div class="h-full flex flex-col items-center justify-center text-center opacity-40 select-none">
                ...
                <h3 class="text-lg font-bold text-slate-700">איך אוכל לסייע לך?</h3>
                <p class="text-xs text-slate-500 max-w-xs mt-2">העלה דוח כספי ב-PDF ושאל שאלות על הכנסות, סיכונים או תחזיות.</p>
              </div>
            }
            ...
            @for (msg of messages(); track $index) {
              <div [class]="msg.role === 'user' ? 'flex justify-end' : 'flex justify-start'" class="animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div [class]="msg.role === 'user' 
                  ...
                  <p class="text-sm leading-relaxed text-right whitespace-pre-wrap">{{ msg.text }}</p>
                  <div class="mt-2 flex items-center gap-1.5 opacity-60 text-[9px] uppercase tracking-tighter justify-end">
          ...
          <div class="p-4 bg-white border-t border-slate-100">
            <div class="relative flex items-center group">
              <input 
                ...
                class="w-full bg-slate-50 border border-slate-200 rounded-xl py-3.5 px-5 pr-4 pl-14 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 focus:bg-white transition-all text-right disabled:opacity-50 disabled:cursor-not-allowed">
              
              <button 
                (click)="ask()"
                [disabled]="!queryText().trim() || !isIndexed() || isAsking()"
                class="absolute left-2.5 p-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-slate-300 disabled:cursor-not-allowed transition-all shadow-md active:scale-95 shadow-blue-100">
...
export class App implements AfterViewChecked {
  ...
  upload() {
    ...
    this.http.post(`${this.api}/upload`, formData).subscribe({
      next: () => {
        ...
        this.addMessage('ai', `הדוח "${file.name}" אונדקס בהצלחה במנוע ה-RAG. אני מוכן לענות על כל שאלה בנוגע לנתונים.`);
      },
      error: (err) => {
        this.isUploading.set(false);
        this.addMessage('ai', 'חלה שגיאה בעיבוד הקובץ. וודא ששרת ה-Backend פועל כראוי בפורט 8000.');
  ...
  ask() {
    ...
    this.http.get<{answer: string}>(`${this.api}/query`, { params: { question: q } }).subscribe({
      ...
      error: (err) => {
        this.isAsking.set(false);
        this.addMessage('ai', 'מצטער, הייתה שגיאה בשליפת המידע מהדוח. וודא שה-Backend פועל.');
